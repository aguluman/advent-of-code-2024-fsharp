...
let part2 (initialSecrets: int64[]) =
    // Reduce thread contention with batched updates
    let localResults =
        initialSecrets
        |> Array.Parallel.map (fun buyerSecret ->
            // Local dictionary for each thread
            let localPatternMap = Dictionary<int, int64>()
            let seenPatterns = HashSet<int>()
            ...
            let patternToInt (c1, c2, c3, c4) =
                // Range of each change is -9 to 9 (19 possibilities)
                // Use bit shifting to create a unique integer
                (int c1 + 9)
                + (int c2 + 9) * 19
                + (int c3 + 9) * 19 * 19
                + (int c4 + 9) * 19 * 19 * 19
            ...
            // Initial pattern
            let mutable patternInt =
                patternToInt (changes[0], changes[1], changes[2], changes[3])

            // Process first pattern
            if not (seenPatterns.Contains patternInt) then
                seenPatterns.Add(patternInt) |> ignore
                localPatternMap[patternInt] <- prices[4]

            // Process remaining sequences with batch pre-generation
            let batchSize = 100

            for batchStart in [ 5..batchSize..2000 ] do
                let batchEnd = min 2000 (batchStart + batchSize - 1)

                // Pre-generate prices for this batch
                let batchPrices = Array.zeroCreate (batchEnd - batchStart + 1)
                let mutable currentForBatch = current

                for i = 0 to batchPrices.Length - 1 do
                    batchPrices[i] <- currentForBatch % 10L
                    currentForBatch <- next currentForBatch

                // Update current to the last computed value
                current <- currentForBatch

                // Process batch
                for i = 0 to batchEnd - batchStart do
                    let price = batchPrices[i]
                    ...
                    patternInt <- patternToInt (changes[0], changes[1], changes[2], changes[3])

                    // Update pattern map
                    if not (seenPatterns.Contains patternInt) then
                        seenPatterns.Add(patternInt) |> ignore

                        match localPatternMap.TryGetValue(patternInt) with
                        | true, existing -> localPatternMap[patternInt] <- existing + price
                        | false, _ -> localPatternMap[patternInt] <- price

            localPatternMap)

    // Merge local results
    let finalPatternMap = Dictionary<int, int64>()

    for localMap in localResults do
        for kvp in localMap do
            match finalPatternMap.TryGetValue(kvp.Key) with
            | true, existing -> finalPatternMap[kvp.Key] <- existing + kvp.Value
            | false, _ -> finalPatternMap[kvp.Key] <- kvp.Value

    // Find maximum value
    let mutable maxValue = 0L

    for KeyValue(_, value) in finalPatternMap do
