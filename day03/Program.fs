...
let part1(multiples: (int * int) seq) =
    multiples |> Seq.sumBy (fun (x, y) -> x * y)
...
let parseMul (text: string) = 
    ...
    |> Option.bind (fun (_, rest) -> 
        parseNumber rest)
    |> Option.bind (fun (x, rest) -> 
        parsePrefix "," rest |> Option.map (fun (_, rest)-> (x, rest)))
    |> Option.bind (fun (x, rest) ->
        parseNumber rest |> Option.map (fun (y, rest) -> ((x, y), rest)))
    |> Option.bind (fun ((x, y), rest ) -> 
        parsePrefix ")" rest |> Option.map (fun (_, rest) -> ((x, y), rest)))
